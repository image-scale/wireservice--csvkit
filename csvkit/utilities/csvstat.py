...
from csvkit.utilities.csvutil import CSVKitUtility, default_float_decimal, parse_column_identifiers
...
class CSVStat(CSVKitUtility):
    ...
    def main(self):
        ...
        operations = [op for op in OPERATIONS.keys() if getattr(self.args, op + '_only', False)]
    ...
    def print_json(self, table, column_ids, stats):
        """
        Print data for all statistics as JSON text.
        """
...
def format_decimal(d, f='%.3f', no_grouping_separator=False):
    # Extract precision from format string like '%.3f'
    match = re.match(r'%\.(\d+)f', f)
    precision = int(match.group(1)) if match else 3

    if no_grouping_separator:
