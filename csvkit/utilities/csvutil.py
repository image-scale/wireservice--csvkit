...
class CSVKitUtility:
    ...
    def launch_new_instance(cls):
        """
        Create and run an instance of the utility.
        """
        utility = cls()
        utility.run()
    ...
    def _init_common_parser(self):
        ...
        if 'p' not in self.override_flags:
            self.argparser.add_argument(
                ...
                help='Character used to escape the delimiter if --quoting 3 ("quote none") is specified.')
        ...
        if 'add-bom' not in self.override_flags:
            self.argparser.add_argument(
                ...
                help='Add the UTF-8 byte-order mark (BOM) to the output.')

        # Input/Output
        if 'zero' not in self.override_flags:
            self.argparser.add_argument(
                '--zero', dest='zero_based', action='store_true',
                help='When interpreting or displaying column numbers, use zero-based numbering.')
    ...
    def _extract_csv_reader_kwargs(self):
        ...
        field_size_limit = getattr(self.args, 'field_size_limit', None)
    ...
    def get_column_types(self):
        ...
        if getattr(self.args, 'no_inference', None):
            ...
            if getattr(self.args, 'out_quoting', None) == 2:  # QUOTE_NONUMERIC
                ...
                if getattr(self.args, 'datetime_format', None):
                    types.insert(-1, number_type)
                elif getattr(self.args, 'date_format', None):
    ...
    def print_column_names(self):
        """
        Pretty-prints the names and indices of all columns to a file-like object.
        """
        if getattr(self.args, 'no_header_row', None):
