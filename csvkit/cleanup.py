...
class RowChecker:
    ...
    def checked_rows(self):
        ...
        for row in self.reader:
            ...
            if self.fill_short_rows:
                if len(row) < len_column_names:
                    row = row + [self.fillvalue] * (len_column_names - len(row))
        ...
        if row_count:  # Don't report all columns as empty if there are no data rows.
            empty_column_indices = [i for i, count in enumerate(empty_counts) if count == row_count]
            if empty_column_indices:
                offset = 0 if self.zero_based else 1
                self.errors.append(
                    Error(
                        1,
                        ["" for _ in range(len_column_names)],
                        f"Empty columns named {', '.join(repr(self.column_names[i]) for i in empty_column_indices)}! "
                        f"Try: csvcut -C {','.join(str(i + offset) for i in empty_column_indices)}",
                    )
                )
